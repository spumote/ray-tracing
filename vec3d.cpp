...
double Vec3d::distToPoint(const Vec3d& a) const {
    return sqrt((a.x - x) * (a.x - x) + (a.y - y) * (a.y - y) + (a.z - z) * (a.z - z));
}

double Vec3d::getCos(const Vec3d& a) const {
    return ((*this) * a) / (*this).len() / a.len();
}

Vec3d Vec3d::getRefraction(const Vec3d& normal) const {
/*	std::cerr << (*this) << std::endl;
	std::cerr << normal << std::endl;
	std::cerr << normal * (*this) << std::endl;
	std::cerr << normal * (normal * (*this)) << std::endl;
*/
	Vec3d normal2 = normal.normalized();
	Vec3d tec = this->normalized();
	return (tec - normal2 * (normal2 * (tec)) * 2).normalized(); 	
}
